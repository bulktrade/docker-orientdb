...
ORIENTDB_DB_USER=${ORIENTDB_DB_USER-dbuser}
QORIENTDB_DB_USER_PASSWORD=`echo $(printf '%q' $ORIENTDB_DB_USER)`
export ORIENTDB_DB_USER
...
sed -i "/<storages>/a <storage name=\"$QORIENTDB_DB\" path=\"plocal:$QORIENTDB_DB\" userName=\"$QORIENTDB_DB_USER\" userPassword=\"$QORIENTDB_DB_USER_PASSWORD\" loaded-at-startup=\"true\" />" /orientdb/config/orientdb-server-config.xml
